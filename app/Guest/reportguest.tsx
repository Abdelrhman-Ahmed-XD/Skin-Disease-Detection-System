...
// ── shared camera-lock geometry (must match GuestBottomNav) ───
  ...
  const renderCameraModal = () => {
    ...
    return (
      <View style={[StyleSheet.absoluteFill, cm.root]} pointerEvents="box-none">
        <TouchableOpacity
          style={[StyleSheet.absoluteFill, cm.overlay]}
          activeOpacity={1}
        ...
        <Animated.View style={[cm.cardWrapper, { opacity: cameraFade, transform: [{ scale: cameraScale }] }]}>
          <View style={cm.card}>
            ...
            {!isLocked && (
              <TouchableOpacity
                style={[cm.primaryBtn, { marginBottom: 8 }]}
                activeOpacity={0.85}
            ...
            <View style={cm.actions}>
              <TouchableOpacity
                style={[cm.actionBtn, { backgroundColor: '#004F7F' }]}
                activeOpacity={0.85}
              ...
              <TouchableOpacity
                style={[cm.actionBtn, { backgroundColor: '#C5E3ED' }]}
                activeOpacity={0.85}
    ...
    <SafeAreaView style={[styles.container, { backgroundColor: pageBg }]} edges={['top']}>
      ...
      <View style={styles.lockContainer}>
        <View style={[styles.iconWrap, { backgroundColor: isDark ? '#fff' : '#004F7F' }]}>
          <Ionicons name="document-lock-outline" size={48} color={isDark ? '#004F7F' : '#fff'} />
        </View>
        <Text style={[styles.lockTitle, { color: isDark ? '#fff' : '#000' }]}>Reports Unavailable</Text>
      ...
      <GuestBottomNav activeTab="Reports" onCameraPress={openCameraModal} />
      {renderCameraModal()}
    </SafeAreaView>
  );
}
...
const cm = StyleSheet.create({
  ...
  card:        { backgroundColor: '#004F7F', borderRadius: 18, padding: 14, shadowColor: '#000', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.35, shadowRadius: 14, elevation: 14 },
